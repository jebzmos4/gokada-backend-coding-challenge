...
/**
  ...
  login(req, res) {
    ...
    const errors = utils.checkRequestBody(req.body, ['atmCard', 'pin']);
    if (errors) {
    ...
    return this.service.login(req.body)
      .then((bankResponse) => {
        ...
        if (bankResponse[0].cardDetails === req.body.atmCard) {
  ...
  enquiry(req, res) {
    ...
    const errors = utils.checkRequestBody(req.body, ['transactionToken']);
    if (errors) {
      this.logger.error('required parameter not passed');
      return Response.failure(res, {
        message: 'required param not found',
    ...
    return this.service.balanceEnquiry(req.body).then((response) => {
      if (response && response.amount) {
        return Response.success(res, {
          message: 'User Balance successfully returned',
